...
class _ForecastScreenState extends State<ForecastScreen> {
  ...
  Future pageViewWeather(index) async{
    setState(() {
        //visibility_forecast=false;
        this.temp = 0;
        this.description = '';
    ...
    http.Response response = await http.get(
      "https://api.openweathermap.org/data/2.5/weather?lat=${this._items[(this._items.length-1-index)][2]}&lon=${this._items[(this._items.length-1-index)][3]}&lang=pt_br&units=metric&appid=${codigo}");
    var results = jsonDecode(response.body);

    http.Response forecast = await http.get(
        "https://api.openweathermap.org/data/2.5/onecall?lat=${this._items[(this._items.length-1-index)][2]}&lon=${this._items[(this._items.length-1-index)][3]}&exclude={current,minutely,hourly,alerts}&appid=${codigo}&lang=pt_br&units=metric");
    forecast_data = jsonDecode(forecast.body);
    print(forecast_data);

    final coordinates = new Coordinates(this._items[(this._items.length-1-index)][2], this._items[(this._items.length-1-index)][3]);
    adresses = await Geocoder.local.findAddressesFromCoordinates(coordinates);
    first = adresses.first;
    print("${first.featureName} : ${first.addressLine}");
    print(first.addressLine.split(',')[0]);
    ...
    setState(() {
      prefs.setString('latitude_escolhida', this._items[(this._items.length-1-index)][2].toString());
      prefs.setString('longitude_escolhida', this._items[(this._items.length-1-index)][3].toString());
      this.temp = results['main']['temp'];
      this.description = results['weather'][0]['description'];
      this.currently = results['weather'][0]['main'];
      this.humidity = results['main']['humidity'];
